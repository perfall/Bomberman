...
import bomberman.AbstractCharacter.Move;
...
/**
 * This class is the biggest class in the game and handles the main logics ad mechanics. The floor is represented
 * as a two-dimensional array that consists of FloorTile, as well as a pixel-based plane that the Abstractcharacters
 * are navigating in, because of this, converter-methods are included in the Floor-class. The "spawning"-methods are
 * included in the class, in other words the player, the enemies, the bombs and explosions are all created, and
 * potentially removed here via a bunch of different methods. The class partially handles moving aspects of the
 * AbstractCharacters and bomb-drops. Collision-methods are handled here and finally the boolean isGameOver is
 * initialised and modified here.
 */
public class Floor {
    ...
    // This method is static since it is a general converter which is not dependent on the instantiated object.
	...
	LOGGER.log(Level.FINE, "Player in explosion, leads to gameOver:", isGameOver);
    }
	...
	LOGGER.log(Level.FINER, "spawning enemies", new Object[]{ enemyList.toString()} );
    }
	...
	for (Bomb bomb : bombList) {
	    LOGGER.log(Level.FINER, "processing bombs in bombList", new Object[]{ bombList.indexOf(bomb), bomb } );
	...
	LOGGER.log(Level.FINE, "Distance between the two circles", c );
